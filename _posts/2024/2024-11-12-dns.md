...
## 주요 DNS 목록
![dnsList](https://yuuuuuuyu.github.io/assets/img/posts/dns1.png)

## DNS 서버 유형
![dnsServer](https://yuuuuuuyu.github.io/assets/img/posts/dns0.png)

사용자 검색을 IP 주소로 변환하는 과정에는 네 가지 유형의 DNS 서버가 관여한다. 

### **1. 재귀 DNS 서버 (Recursive Resolver)**
...
### **<a href="https://ko.wikipedia.org/wiki/루트_네임_서버" target="_blank">2. 루트 네임 서버</a>**
...
## <a href="https://www.cloudflare.com/ko-kr/learning/dns/what-is-dns/" target="_blank">DNS 조회 과정</a>
![dnsLookupCloudflare](https://yuuuuuuyu.github.io/assets/img/posts/dns2.png)

위 그림은 `example.com`을 입력했을 때 DNS를 조회하는 단계를 나타낸 것이다.

**1. 도메인 입력**
: 사용자가 웹 브라우저에 `example.com`을 입력한다.

**2. DNS Resolver 요청**
: 쿼리가 인터넷을 통해 DNS Resolver에 전달된다.

**3. 루트 서버 질의**
: Resolver가 루트 서버에 질의한다.

**4. TLD 서버 응답**
: 루트 서버는 .com TLD 서버의 주소를 반환한다.

**5. TLD 서버 질의**
: Resolver가 .com TLD 서버에 요청을 보낸다.

**6. 권한 있는 네임 서버 응답**
: TLD 서버는 `example.com`의 권한 있는 네임 서버 주소를 제공한다.

**7. 권한 있는 네임 서버 질의**
: Resolver가 권한 있는 네임 서버에 질의하여 IP 주소를 얻는다.

**8. IP 주소 반환**
: 권한 있는 네임 서버가 IP 주소를 확인자에게 반환하고, Resolver가 이를 브라우저에 전달한다. 이 단계를 거쳐서 `example.com`의 IP 주소가 반환되면, 이제 브라우저가 웹 페이지를 요청할 수 있다.
