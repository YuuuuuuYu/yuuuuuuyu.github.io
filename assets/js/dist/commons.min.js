...
!function() {
    ...
    class r {
        static toggle() {
            !1 === r.isExpanded ? o.attr(t, "") : o.removeAttr(t),
    ...
    !function() {
        const e = $(window)
          , s = $("#back-to-top");
        e.on("scroll", (()=>{
            e.scrollTop() > 50 ? s.fadeIn() : s.fadeOut()
        }
        )),
        s.on("click", (()=>{
            e.scrollTop(0)
    ...
    0 !== e.length && e.off().on("click", (e=>{
        const s = $(e.target);
        let o = s.prop("tagName") === "button".toUpperCase() ? s : s.parent();
        modeToggle.flipMode(),
        o.trigger("blur")
    }
    )),
    ...
    p.on("input", (()=>{
        "" === p.val() ? T() ? g.removeClass(v) : k.off() : (k.on(),
        T() && g.addClass(v))
    }
    ))
}();
